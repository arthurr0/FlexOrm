...
import pl.minecodes.orm.annotation.*;
...
class TableMetadataTest {
    ...
    void testTableNameFromAnnotation() {
        EntityRepository<CustomTableNameEntity, Long> agent = flexOrm.getEntityRepository(CustomTableNameEntity.class);

        agent.executeUpdate("CREATE TABLE IF NOT EXISTS custom_table (id INTEGER PRIMARY KEY, value TEXT)");
    ...
    void testDefaultTableName() {
        EntityRepository<DefaultTableEntity, Long> agent = flexOrm.getEntityRepository(DefaultTableEntity.class);

        agent.executeUpdate("CREATE TABLE IF NOT EXISTS defaulttableentity (id INTEGER PRIMARY KEY, name TEXT)");
    ...
    void testCustomColumnName() {
        EntityRepository<CustomColumnEntity, Long> agent = flexOrm.getEntityRepository(CustomColumnEntity.class);

        agent.executeUpdate("CREATE TABLE IF NOT EXISTS custom_column_test (id INTEGER PRIMARY KEY, user_name TEXT)");
    ...
    void testTransientFieldExcluded() {
        EntityRepository<TransientFieldEntity, Long> agent = flexOrm.getEntityRepository(TransientFieldEntity.class);

        agent.executeUpdate("CREATE TABLE IF NOT EXISTS transient_test (id INTEGER PRIMARY KEY, name TEXT)");
    ...
    void testEntityWithoutOrmEntityAnnotation() {
        assertThrows(RuntimeException.class, () -> {
            EntityRepository<NoAnnotationEntity, Long> agent = flexOrm.getEntityRepository(NoAnnotationEntity.class);
            agent.query();
        });
    }
    ...
    void testRelationDetection() {
        EntityRepository<EntityWithRelations, Long> agent = flexOrm.getEntityRepository(EntityWithRelations.class);
    ...
    void testTableMetadataWithRelations() throws NoSuchFieldException {
        ...
        TableMetadata metadata = new TableMetadata("test", idField, columnFields, fieldColumnNames, List.of());
    ...
    public static class TransientFieldEntity {
        ...
        public void setTransientValue(String transientValue) { this.transientValue = transientValue; }
    }
